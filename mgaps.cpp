...
//  Process the cluster of matches in  A [0 .. (N - 1)].
//  Return the number of clusters printed.
         ...
         for  (j = 0;  j < i;  j ++)
           {
            ...
            Pen = Olap + abs ( (A [i] . Start2 - A [i] . Start1) - (A [j] . Start2 - A [j] . Start1) );
   ...
   for  (i = 1;  i <= N;  i += cluster_size)
     {
      ...
      for  (j = i + 1;  j <= N && A [i] . cluster_id == A [j] . cluster_id;  j ++)
        ;
      cluster_size = j - i;
      print_ct += Process_Cluster (A + i, cluster_size);
     }
